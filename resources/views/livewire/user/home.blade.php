    ...
    @forelse($quizzes as $quiz)
        <div class="w-full bg-black text-black-900 py-3 px-6 rounded border shadow relative mb-4">
            <h3 class="text-lg font-bold pr-12">
                <a href="{{ route('user.quizzes.manage', $quiz->id) }}">{{ $quiz->title }}</a>
            </h3>
            <p class="mt-2">
                @if(! $isFresh)
                    <button wire:click="startSession({{ $quiz->id }})"
                        class="px-2 py-1 text-sm rounded bg-blue-500 hover:bg-blue-700 text-white font-bold">
                        Start
                    </button>
                ...
                @else
                    <a href="{{ route('user.quiz.start', $sessions) }}"
                        class="ml-2 px-2 py-1 text-sm rounded bg-orange-500 hover:bg-orange-700 text-black font-bold">
                        Resume
                    </a>    
                    <button wire:click="abandonAndStartNewSession({{ $quiz->id }})"
                        class="ml-2 px-2 py-1 text-sm rounded bg-orange-500 hover:bg-orange-700 text-black font-bold">
                        Abandon and Start New
                    </button>
                    <button wire:click="discardSession({{ $quiz->id }})"
                        class="ml-2 px-2 py-1 text-sm rounded bg-orange-500 hover:bg-orange-700 text-black font-bold">
                        Discard Session
                    </button>
                    <button wire:click="deleteQuiz({{$quiz->id}})"

<div class="add-question">
    ...
    <form wire:submit.prevent="create">
        ...
        <div class="mb-2">
            ...
            <input id="time_limit" type="number" wire:model="timeLimit" class="w-full px-3 py-2 border rounded bg-black">
            @error('timeLimit')
            <p class="text-red-600 mt-2">{{ $message }}</p>
            @enderror
        </div>
            ...
            @foreach($options as $index => $option)
                <div class="flex items-center mb-2" wire:key="{{$loop->index}}">
                    ...
                    <input type="text" wire:model="options.{{ $index }}"
                        class="w-full px-3 border rounded bg-black text-white flex-1 mr-2 mb-1"
